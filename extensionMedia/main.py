...
class main_window(QMainWindow):
    def __init__(self):
        ...
        self.L = 1
        self.A = 0
        self.informationSlider.setTickPosition(int(self.A*1000))
        self.informationSlider.setSliderPosition(int(self.A*1000))
        self.legitimacySlider.setTickPosition(int(self.L*100))
        self.legitimacySlider.setSliderPosition(int(self.L*100))
        self.maxJail = 30
        self.actives = []
        self.nbrAct = 0
        self.CopNumber = 0
    ...
    def connectUI(self):
        ...
        self.legitimacySlider.valueChanged.connect(self.updateLegitimacy)
        self.informationSlider.valueChanged.connect(self.updateInformation)
        ...
        for i in np.arange(self.agentNumber):
            self.agents.append(
                toolbox.agent([locations[0][i], locations[1][i]], Hs[i], Rs[i], self.L,
                    self.A, self.agentVis, self.field, self.nbrAct, self.CopNumber, self.maxJail))
    ...
    def updateView(self):
        ...
        try:
            self.nbrAct = sum(self.fields[-1].flatten() == 'a')
            self.actives.append(self.nbrAct)
            self.curveActive.setData(x, self.actives)
        except IndexError:
            pass
        self.curveJailed.setData(x, self.jailed_agents)

    def updateLegitimacy(self):
        """This function handles the legitimacy slider"""
        newL = self.legitimacySlider.value()
        self.L = newL/100
    ...
    def updateInformation(self):
        """This function handles the legitimacy slider"""
        newA = self.informationSlider.value()
        self.A = newA/1000
        for a in self.agents:
            a.A = self.A
